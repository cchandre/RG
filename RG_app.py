...
def main():
	...
	mpc_positions = (11, 0), (11, 1)

	tol_names = 'TolMin', 'TolMax', 'TolLie', 'MaxIter', 'MaxLie', 'DistSurf', 'NumberOfIterations'
	tol_types = 'Double', 'Double', 'Double', 'Int', 'Int', 'Double', 'Int'
	tol_values = 1e-9, 1e+9, 1e-11, 5000, 5000, 1e-7, 10
	tol_positions = (1, 3), (2, 3), (5, 3), (3, 3), (6, 3), (8, 3), (8, 0)
	...
	for (run_etiq, run_position) in zip(run_etiqs, run_positions):
		b_method = tk.Radiobutton(tab_run, variable=run_method, text=run_etiq, value=run_etiq, width=15, anchor='w')
	...
	tk.Button(tab_run, text='Run', command= lambda : rg_run(run_method, case_var, mp_names, mp_par, mpc_names, mpc_par, adv_names, adv_par, adv2_names, adv2_par, tol_names, tol_par)).grid(row=11, column=4)
...
def makeform(root, fields, names, positions):
	...
	for (field, name, position) in zip(fields, names, positions):
		lab = tk.Label(root, width=15, text=name, anchor='e')
		ent = tk.Entry(root, width=10, textvariable=field)
...
def rg_run(run_method, case_var, mp_names, mp_par, mpc_names, mpc_par, adv_names, adv_par, adv2_names, adv2_par, tol_names, tol_par):
	params = dict()
	...
	N, omega_0, Eigenvalues, Omega, K, KampInf, KampSup = define_case(case_var.get())
	case_study = RG.RGcase(N, omega_0, params)
	modes_i = RG.ModesInit(K, KampInf, KampSup, Omega)
	if run_method.get() == 'Iterates':
		case_study.iterates(modes_i)
	elif run_method.get() == 'Circle':
		case_study.iterate_circle(modes_i)
	elif run_method.get() == 'Critical Surface':
		case_study.critical_surface(modes_i)
	elif run_method.get() == 'Converge Region':
		case_study.converge_region(modes_i)
...
def define_case(case):
	if case == 'GoldenMean':
	    ...
	    KampInf = [0.0, 0.0, 0.0]
	    KampSup = [0.1, 0.1, 0.1]
	return N, omega_0, Eigenvalues, Omega, K, KampInf, KampSup
